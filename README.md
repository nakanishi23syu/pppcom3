...
再現した学習用プロジェクトです。詳細な経緯は [`実務環境再現_提案書.md`](./docs/実務環境再現_提案書.md) を参照してください。

> **状態**: 全サービス実装済み。各サービスは個別にビルド・実機検証済みです（詳細は
> [`docs/architecture.md`](./docs/architecture.md) 5章の検証状況一覧を参照）。
> 手を動かす前に必ず [`手動セットアップ手順.md`](./docs/手動セットアップ手順.md) を読んでください
...
詳しい手順・前提ソフトのインストールは [`手動セットアップ手順.md`](./docs/手動セットアップ手順.md) を参照。
実務相当のVM(Windows Server 2025 + IIS)環境をゼロから構築したい場合は
[`VM構築手順.md`](./docs/VM構築手順.md) を参照（ローカルのdocker-compose環境は使わず、VM上に
